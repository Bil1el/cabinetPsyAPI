...
| `PATCH /api/psychologist/profile` | Sanctum; current psychologist policy | Optional profile fields: names, unique `email`, `phone`, `photo`, `speciality`, `bio`, `consultation_duration` (15–240), `is_active` | Updated private psychologist profile |
| `GET /api/working-hours` | Sanctum; current psychologist policy | None | Working-hour collection: `id`, `dayOfWeek`, `startsAt`, `endsAt`, `isActive` |
| `PUT /api/working-hours` | Sanctum; current psychologist policy | Required `ranges` array (maximum 28); each range has weekday enum, `starts_at`, `ends_at` (`HH:MM`, end after start), optional `is_active` | Replaced working-hour collection; `409 SCHEDULE_CONFLICT` when protected future appointments would no longer fit |
